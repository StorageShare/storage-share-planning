        ...
        <div class="sm:flex sm:items-center sm:justify-between mb-6">
            <div>
                <div class="flex items-center gap-x-3">
                    <h2 class="text-lg font-medium text-gray-800 dark:text-white">Gebruiker Bewerken</h2>
                </div>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">Bewerk de gegevens van de gebruiker.</p>
            </div>
        </div>
            ...
            <div class="p-6 text-gray-900 dark:text-gray-100">
                <form action="{{ route('users.update', $user) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <x-text-input type="text" id="name" name="name" class="block mt-1 w-full" value="{{ $user->name }}" disabled />
                    </div>

                    <div>
                        <x-input-label for="email" :value="__('Email')" />
                        <x-text-input type="email" id="email" name="email" class="block mt-1 w-full" value="{{ $user->email }}" disabled />
                    </div>
